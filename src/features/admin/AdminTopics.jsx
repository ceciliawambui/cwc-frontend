...
// import client from "../../features/auth/api"; // your axios instance with auth token
...
// // Helper to extract plain text from tiptap JSON (for search/preview)
...
//   // create editor — NOTE: StarterKit is the base; we only add extra extensions we need
//   const editor = useEditor({
//     extensions: [
//       StarterKit.configure({ codeBlock: false }), // we use CodeBlockLowlight instead
//       Underline,
//       Placeholder.configure({ placeholder: "Start writing — paste formatted text, add code, tables, media…" }),
//       ImageExt,
...
//     },
//     onUpdate: ({ editor }) => {
//       // Immediate meta.content update so editor changes are saved live
...
//     // ensure editor cleared
//     setTimeout(() => editor?.commands.setContent(blank.content), 50);
//     setShowModal(true);
//   };
...
//     setTimeout(() => {
//       try {
//         if (topic.content && editor) editor.commands.setContent(topic.content);
//       } catch (e) {
...
//   // Insert code block with language attribute
//   const insertCodeBlockWithOptions = ({ language = "javascript", runnable = false }) => {
//     const sample = language === "html" ? "<!-- html here -->\n" : "// example\n";
//     editor
//       ?.chain()
//       .focus()
//       .insertContent({
//         type: "codeBlock",
//         attrs: { language, runnable: runnable ? true : false },
//         content: [{ type: "text", text: sample }],
//       })
//       .run();
//     // move cursor inside codeblock
//     setTimeout(() => editor?.chain().focus().run(), 20);
//   };

//   // Insert table (uses table commands if available)
//   const insertTable = (rows = 2, cols = 3, withHeader = true) => {
//     try {
//       editor.chain().focus().insertTable({ rows, cols, withHeaderRow: withHeader }).run();
//     } catch (err) {
//       // fallback: basic table node insertion (some setups may not expose insertTable)
//       editor.chain().focus().insertContent({
//         type: "table",
//         content: [],
//       }).run();
//       toast.success("Table inserted (basic).");
//     }
//   };

//   // Paste handler — prefer rich HTML, fallback to plain text
...
//       try {
//         // TipTap can parse HTML via setContent/insertContent
...
//     e.preventDefault();
//     if (!meta.title.trim() || !meta.course) return toast.error("Title and Course are required.");
//     setSaving(true);
//     const payload = {
//       title: meta.title,
...
//           <p className="text-sm text-gray-600 mt-1">Rich editor: highlight text to format, paste preserved formatting, add tables, media & code blocks.</p>
//         </div>

//         <div className="flex gap-2">
//           <div className="flex items-center bg-gray-50 border rounded-md px-3 py-2 gap-2 w-[260px]">
//             <Search className="text-gray-400" />
//             <input value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search topics..." className="bg-transparent outline-none w-full text-sm" />
//           </div>

//           <select value={courseFilter} onChange={(e) => setCourseFilter(e.target.value)} className="border rounded-md px-3 py-2 text-sm">
//             <option value="">All Courses</option>
//             {courses.map((c) => <option key={c.id} value={c.id}>{c.title}</option>)}
//           </select>

//           <button onClick={openAddModal} className="bg-indigo-600 text-white px-3 py-2 rounded-md hover:bg-indigo-700 flex items-center gap-2"><Plus size={14} /> New</button>
//         </div>
//       </div>
...
//           <tbody>
//             {loading ? (
//               <tr><td colSpan={4} className="text-center py-6 text-gray-500"><Loader2 className="animate-spin inline-block" /> Loading...</td></tr>
//             ) : paginated.length === 0 ? (
//               <tr><td colSpan={4} className="text-center py-6 text-gray-500">No topics found.</td></tr>
//             ) : (
//               paginated.map((t) => (
//                 <tr key={t.id} className="border-t hover:bg-gray-50 transition">
//                   <td className="px-4 py-3 font-medium">{t.title}</td>
//                   <td className="px-4 py-3 text-sm text-gray-600">{t.course?.title || "-"}</td>
//                   <td className="px-4 py-3 text-sm text-gray-600">{previewFromContent(t.content)}</td>
//                   <td className="px-4 py-3 text-right">
//                     <button onClick={() => openEditModal(t)} className="px-2 text-indigo-600 hover:underline"><Edit3 size={14} /> Edit</button>
//                     <button onClick={() => promptDelete(t)} className="px-2 text-red-600 hover:underline"><Trash2 size={14} /> Delete</button>
//                   </td>
//                 </tr>
//               ))
//             )}
...
//           <button onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page === 1} className="border rounded px-3 py-1 disabled:opacity-40"><ChevronLeft size={16} /></button>
//           <span>{page}/{totalPages}</span>
//           <button onClick={() => setPage((p) => Math.min(totalPages, p + 1))} disabled={page === totalPages} className="border rounded px-3 py-1 disabled:opacity-40"><ChevronRight size={16} /></button>
//         </div>
//       </div>
...
//             <motion.div className="absolute inset-0 bg-black/40" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={closeModal} />

//             <motion.div initial={{ y: 20, opacity: 0, scale: 0.98 }} animate={{ y: 0, opacity: 1, scale: 1 }} exit={{ y: 10, opacity: 0, scale: 0.98 }} className="relative w-full max-w-6xl max-h-[90vh] overflow-y-auto bg-white rounded-lg shadow-xl border border-gray-200">
...
//                 <div>
//                   <label className="block text-sm text-gray-700 mb-1">YouTube / Vimeo URL</label>
...
//                 {/* Toolbar */}
//                 <div className="flex flex-wrap items-center gap-2 bg-gray-50 border rounded px-3 py-2 sticky top-0 z-10">
...
//                   <button type="button" onClick={() => editor?.chain().focus().toggleUnderline().run()} className="px-2 py-1 border rounded text-sm bg-white"><span className="font-semibold">U</span></button>
...
//                   <button type="button" onClick={() => editor?.chain().focus().toggleBlockquote().run()} className="px-2 py-1 border rounded text-sm bg-white">❝</button>

//                   <button type="button" onClick={() => editor?.chain().focus().setTextAlign("left").run()} className="px-2 py-1 border rounded text-sm bg-white">Left</button>
//                   <button type="button" onClick={() => editor?.chain().focus().setTextAlign("center").run()} className="px-2 py-1 border rounded text-sm bg-white">Center</button>
//                   <button type="button" onClick={() => editor?.chain().focus().setTextAlign("right").run()} className="px-2 py-1 border rounded text-sm bg-white">Right</button>

//                   <button type="button" onClick={() => { const url = window.prompt("Image URL"); if (url) editor?.chain().focus().setImage({ src: url }).run(); }} className="px-2 py-1 border rounded text-sm bg-white"><ImageIcon size={14} /></button>

//                   <button type="button" onClick={() => { const url = window.prompt("YouTube or Vimeo URL"); if (url) editor?.chain().focus().setYoutubeVideo({ src: url }).run(); }} className="px-2 py-1 border rounded text-sm bg-white"><YoutubeIcon size={14} /></button>

//                   <button type="button" onClick={() => insertTable(2, 3, true)} className="px-2 py-1 border rounded text-sm bg-white"><TableIcon size={14} /> Table</button>

//                   <button type="button" onClick={() => { const href = window.prompt("Link URL"); if (href) editor?.chain().focus().setLink({ href }).run(); }} className="px-2 py-1 border rounded text-sm bg-white"><LinkIcon size={14} /></button>

//                   {/* Code insertion */}
//                   <div className="flex items-center gap-2 ml-2">
//                     <select value={codeLang} onChange={(e) => setCodeLang(e.target.value)} className="border px-2 py-1 rounded text-sm bg-white">
//                       {CODE_LANGS.map((l) => <option key={l} value={l}>{l}</option>)}
//                     </select>
//                     <label className="flex items-center gap-1 text-sm">
//                       <input type="checkbox" checked={codeRunnable} onChange={(e) => setCodeRunnable(e.target.checked)} />
//                       runnable
//                     </label>
//                     <button type="button" onClick={() => insertCodeBlockWithOptions({ language: codeLang, runnable: codeRunnable })} className="px-2 py-1 border rounded text-sm bg-white flex items-center gap-1"><Code size={14} /> Insert</button>
//                   </div>
//                 </div>

//                 {/* Editor */}
//                 <div className="border border-gray-200 rounded h-[520px] overflow-y-auto">
//                   <div onPaste={handlePaste} className="p-3">
...
/**
 * AdminTopics.jsx
 * All-in-one Admin Topics editor (TipTap + listing + modal + preview + runnable code)
 *
 * Requirements:
 * - Vite React app
 * - TailwindCSS + Framer Motion + lucide-react + react-hot-toast installed
 * - @tiptap/react, @tiptap/starter-kit, @tiptap/extension-underline, @tiptap/extension-placeholder,
 *   @tiptap/extension-image, @tiptap/extension-youtube, @tiptap/extension-text-align,
 *   @tiptap/extension-code-block-lowlight, @tiptap/extension-table, @tiptap/extension-table-row,
 *   @tiptap/extension-table-cell, @tiptap/extension-table-header, @tiptap/extension-link,
 *   @tiptap/extension-code, @tiptap/extension-text-style, @tiptap/extension-color,
 *   @tiptap/extension-dropcursor, @tiptap/extension-gapcursor, @tiptap/extension-history,
 *   @tiptap/extension-list-item, @tiptap/extension-bullet-list, @tiptap/extension-ordered-list,
 *   @tiptap/extension-character-count
 * - highlight.js / lowlight languages (included)
 *
 * NOTE: adjust `client` import path to your axios/auth instance.
 */
/* eslint-disable no-unused-vars */
import React, { useEffect, useMemo, useState, useRef } from "react";
