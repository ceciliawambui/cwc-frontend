...
// Single Course Card memoized to avoid rerenders
    ...
    <Link to={`/courses/${course.id}`}>
      ...
      >
        ...
        <div className={`absolute inset-0 opacity-0 group-hover:opacity-10 transition-opacity duration-500
          ${theme === "dark" ? "bg-linear-to-br from-indigo-400 to-pink-400" : "bg-linear-to-br from-indigo-200 to-pink-200"}`}
        />
      </motion.div>
    </Link>
  );
...
export default function CoursesPage() {
  ...
  return (
    <div className={`min-h-screen transition-colors duration-500 ${theme === "dark" ? "bg-linear-to-b from-gray-950 via-gray-900 to-black text-white" : "bg-linear-to-b from-white via-gray-50 to-gray-100 text-gray-900"}`}>
      <div className="container mx-auto px-6 pt-32 pb-12 text-center">
        <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }} className={`text-5xl font-extrabold tracking-tight mb-4 ${theme === "dark" ? "text-gray-300" : "text-gray-600"}`}>
          Explore Inspiring Tech Courses
        </motion.h1>
        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2, duration: 0.7 }} className={`max-w-2xl mx-auto text-lg leading-relaxed ${theme === "dark" ? "text-gray-300" : "text-gray-600"}`}>
      ...
      <div className="container mx-auto px-6 pb-24">
        {loading ? (
          <div className={`text-center py-32 text-lg ${theme === "dark" ? "text-white/70" : "text-gray-600"}`}>Loading courses...</div>
        ) : courses.length === 0 ? (
          <div className="text-center py-32 text-lg text-gray-500">No courses available yet. Check back soon!</div>
        ) : (
          <motion.div initial="hidden" animate="visible" variants={{ hidden: { opacity: 0 }, visible: { opacity: 1, transition: { staggerChildren: 0.1 } } }} className="grid gap-10 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
            {courses.map((course) => (
